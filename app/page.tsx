...
export default function Home() {
  ...
  // FITUR PEMESANAN LANGSUNG KE WHATSAPP
  const handleCheckout = () => {
    ...
    let detailPesanan = `Halo JayaCake Bali, saya ingin memesan:\n\n`;
    let total = 0;
    cart.forEach((item, index) => {
      detailPesanan += `${index + 1}. ${item.nama} (${item.qty}x) - Rp${(item.harga * item.qty).toLocaleString('id-ID')}\n`;
    ...
    // Link WhatsApp
    const phone = "628123456789"; // Ganti dengan nomor Anda
    const waUrl = `https://wa.me/${phone}?text=${encodeURIComponent(detailPesanan)}`;
    ...
    <main className="min-h-screen bg-white text-zinc-950 selection:bg-orange-100 selection:text-orange-600">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 mb-20 items-start">
            ...
            <div className="lg:col-span-5 space-y-8">
              ...
              <div className="flex gap-4 pt-2">
                <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-[#1a1a1a] rounded-xl flex items-center justify-center hover:bg-[#ff5c00] transition-all group cursor-pointer">
                ...
                <a href="https://tiktok.com" target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-[#1a1a1a] rounded-xl flex items-center justify-center hover:bg-[#ff5c00] transition-all group cursor-pointer">
                ...
                <a href="https://wa.me/628123456789" target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-[#1a1a1a] rounded-xl flex items-center justify-center hover:bg-[#ff5c00] transition-all group cursor-pointer">
        ...
        {showSuccess && (
          <div className="fixed inset-0 bg-zinc-950 z-[200] flex items-center justify-center text-center p-6 backdrop-blur-xl">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
              <span className="text-orange-500 font-black text-sm uppercase tracking-[0.6em] mb-6 block">Pesanan Terkonfirmasi & Dikirim ke WA</span>
      ...
      {cart.length > 0 && !showSuccess && (
        <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="fixed bottom-10 right-10 w-[90%] md:w-96 bg-zinc-900 text-white rounded-[2.5rem] p-10 shadow-2xl z-50 border border-zinc-800/50 backdrop-blur-lg">
          ...
          <button onClick={handleCheckout} className="w-full bg-orange-600 text-white py-6 rounded-[1.5rem] font-black text-sm hover:bg-orange-500 transition-all shadow-xl uppercase tracking-[0.2em]">Checkout ke WhatsApp</button>
        </motion.div>
      )}
    </main>
  );
